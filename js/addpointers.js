  $.fn.addListviewPointers = function () {
    ...
    if ( that < $(window).height()) {
      $(document).off('scrollstart.SHOWBTTNS scroll.SHOWBTTNS').on('scrollstart.SHOWBTTNS scroll.SHOWBTTNS', function () {
        hideIndicators();
      });
      hideIndicators();
    }
    // DO apply solution for UL element with the height NOT defined
      ...
      $(document).off('scrollstart.SHOWBTTNS scroll.SHOWBTTNS').on('scrollstart.SHOWBTTNS scroll.SHOWBTTNS', function () {
         ...
         */
                                    ...
                                    && $(document).scrollTop() + $('[data-role="header"]').height() < that.offset().top + that.outerHeight() - $(window).height()) {          
        /*
         ...
         *  component bottom
         */
        else if ($(document).scrollTop() + $('[data-role="header"]').height() >= that.offset().top + that.outerHeight() - $(window).height()) {
